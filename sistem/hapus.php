...
<?php
	...
	if ($cek == "KRY"){
		...
		$sqlfoto 	= "SELECT foto_pembeli FROM pembeli WHERE id_karyawan='".$_GET['id']."'";
		$queryfoto	= mysql_query($sqlfoto);
		$dfoto		= mysql_fetch_array($queryfoto);
		$target = "../aplikasi/foto_pembeli/".$dfoto['foto_pembeli'];
		if(file_exists($target)){
			unlink($target);
		}
		...
		$sql2 	= "DELETE FROM pembeli WHERE id_pembeli='".$_GET['id']."'";
		$query2	= mysql_query($sql2, $server);
		if($query && $query2){
