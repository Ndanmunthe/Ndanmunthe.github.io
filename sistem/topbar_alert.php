...
// $gudang 		= "0:0:1";
	...
	$level 		= $_SESSION['pengusaha_level'];
	if ($level == '1'){
		$view_update = "";
	}else
	if ($level == '3'){
  ...
  <ul id="menu1" class="dropdown-menu list-unstyled msg_list" role="menu">
    <?php
      while($dta_alert = mysql_fetch_array($qry_alert)){
		  ...
		  $dta_alert2 = mysql_fetch_array(mysql_query("SELECT * FROM pembeli WHERE id_pembeli='".$dta_alert['id_pembeli']."'"));
		  
		if ($dta_alert2['foto_pembeli'] == ""){
		$avatar = "../aplikasi/foto_pembeli/unavailable.jpg";
		}else{
		$avatar = "../aplikasi/foto_pembeli/".$dta_alert2['foto_pembeli'];
		}
    ...
    <li>
      <a href="?page=detail-transaksi&order=<?php echo $dta_alert['id_order']?>">
        ...
        <span>
          ...
          <span><?php echo ucwords(strtolower($dta_alert2['nama_pembeli']))?></span>
        </span>
        <span class="message">
