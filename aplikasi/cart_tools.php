...
<?php
	$sqlcekorder 	= "SELECT * FROM orders_items JOIN orders ON orders.id_order = orders_items.id_order WHERE orders.id_pembeli='".$_SESSION['pengusaha_id']."' AND orders.status='1'";
	...
	$sql	= "SELECT * FROM orders JOIN orders_items ON 
						...
						produk.id_produk = orders_items.id_produk WHERE orders.id_pembeli= '".$_SESSION['pengusaha_id']."' AND orders.status='1' AND orders_items.id_produk='$id_produk' ";
	...
	if($id_order!= "" or $id_produk!=""){
		if($j < 2){
			$del2 = mysql_query("DELETE FROM orders_items WHERE 
										id_produk= '".$_GET['produk']."' &&
										id_order = '".$_GET['id']."' ");
			$del = mysql_query("DELETE FROM orders WHERE id_order = '".$_GET['id']."' && id_pembeli='".$_SESSION['pengusaha_id']."'");
			
			
		}else if($j > 1){
			$del2 = mysql_query("DELETE FROM orders_items WHERE id_order = '".$_GET['id']."' && id_produk='$id_produk' && id_produk='$produkid'");
			$sql7 = mysql_query("UPDATE orders SET total='".$GTOT."' WHERE id_order = '".$_GET['id']."' && status ='1' && id_pembeli='".$_SESSION['pengusaha_id']."'");
